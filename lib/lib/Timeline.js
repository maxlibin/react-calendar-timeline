...
var ReactCalendarTimeline = function (_Component) {
  ...
  _createClass(ReactCalendarTimeline, [{
    ...
    value: function rows(canvasWidth, groupHeights, groups) {
      ...
      var currentItem = this.props.items.find(function (item) {
        return _this3.state.selectedItem === item.id;
      });

      var currentItemGroup = currentItem && currentItem.group - 0;
    ...
    value: function render() {
      ...
      var _props = this.props,
          ...
          rightSidebarStack = _props.rightSidebarStack,
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.resize = function () {
    ...
    var _stackItems2 = (0, _calendar.stackItems)(props.items, props.groups, _this5.state.canvasTimeStart, _this5.state.visibleTimeStart, _this5.state.visibleTimeEnd, width, _this5.props, _this5.state),
    ...
    _this5.headerRef.scrollLeft = width;
  };
  ...
  this.onScroll = function (scrollX) {
    ...
    _this5.headerRef.scrollLeft = newScrollX;
    _this5.scrollComponent.scrollLeft = newScrollX;
  ...
  this.updateScrollCanvas = function (visibleTimeStart, visibleTimeEnd, forceUpdateDimensions) {
    var items = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : _this5.props.items;
    var groups = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : _this5.props.groups;

    _this5.setState((0, _calendar.calculateScrollCanvas)(visibleTimeStart, visibleTimeEnd, forceUpdateDimensions, items, groups, _this5.props, _this5.state));
  };
  ...
  this.changeZoom = function (scale) {
    ...
    var newVisibleTimeStart = Math.round(_this5.state.visibleTimeStart + (oldZoom - newZoom) * offset);

    _this5.props.onTimeChange(newVisibleTimeStart, newVisibleTimeStart + newZoom, _this5.updateScrollCanvas);
  };
  ...
  this.selectItem = function (item, clickType, e) {
    if (_this5.state.selectedItem === item || _this5.props.itemTouchSendsClick && clickType === 'touch') {
      if (item && _this5.props.onItemClick) {
        var time = _this5.timeFromItemEvent(e);
        _this5.props.onItemClick(item, e, time);
      }
    } else {
      _this5.setState({ selectedItem: item });
      if (item && _this5.props.onItemSelect) {
        var _time = _this5.timeFromItemEvent(e);
        _this5.props.onItemSelect(item, e, _time);
      } else if (item === null && _this5.props.onItemDeselect) {
        _this5.props.onItemDeselect(e); // this isnt in the docs. Is this function even used?
      }
    }
  };
  ...
  this.dragItem = function (item, dragTime, newGroupOrder) {
    var newGroup = _this5.props.groups[newGroupOrder];
  ...
  this.dropItem = function (item, dragTime, newGroupOrder) {
    _this5.setState({ draggingItem: null, dragTime: null, dragGroupTitle: null });
    if (_this5.props.onItemMove) {
      _this5.props.onItemMove(item, dragTime, newGroupOrder);
    }
  };
  ...
  this.resizedItem = function (item, resizeTime, edge, timeDelta) {
    _this5.setState({ resizingItem: null, resizingEdge: null, resizeTime: null });
    if (_this5.props.onItemResize && timeDelta !== 0) {
  ...
  this.handleRowClick = function (e, rowIndex) {
    ...
    if (_this5.state.selectedItem) {
      _this5.selectItem(null);
    }

    if (_this5.props.onCanvasClick == null) return;

    var time = _this5.getTimeFromRowClickEvent(e);
    var groupId = (0, _generic._get)(_this5.props.groups[rowIndex], _this5.props.keys.groupIdKey);
    _this5.props.onCanvasClick(groupId, time, e);
  };

  this.handleRowDoubleClick = function (e, rowIndex) {
    if (_this5.props.onCanvasDoubleClick == null) return;

    var time = _this5.getTimeFromRowClickEvent(e);
    var groupId = (0, _generic._get)(_this5.props.groups[rowIndex], _this5.props.keys.groupIdKey);
    _this5.props.onCanvasDoubleClick(groupId, time, e);
  };

  this.handleScrollContextMenu = function (e, rowIndex) {
    if (_this5.props.onCanvasContextMenu == null) return;

    var timePosition = _this5.getTimeFromRowClickEvent(e);

    var groupId = (0, _generic._get)(_this5.props.groups[rowIndex], _this5.props.keys.groupIdKey);

    if (_this5.props.onCanvasContextMenu) {
      e.preventDefault();
      _this5.props.onCanvasContextMenu(groupId, timePosition, e);
    }
  };

  this.handleHeaderRef = function (el) {
    _this5.headerRef = el;
    _this5.props.headerRef(el);
  };

  this.handleScrollHeaderRef = function (el) {
    _this5.scrollHeaderRef = el;
  };
};
...
ReactCalendarTimeline.propTypes = {
  ...
  rightSidebarStack: _propTypes2.default.bool,
